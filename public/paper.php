...
function update_paper(Qrequest $qreq, $action) {
    ...
    else if (isset($_GET['corrupt_document']))
        $webmsg = $Conf->_("Submission #%d was successfully corrupted!", $prow->paperId);
    else if (empty($ps->diffs))
        $webmsg = $Conf->_("(1) No changes to submission #%d.", $prow->paperId);
    else
...
if (($Qreq->update || $Qreq->submitfinal) && $Qreq->post_ok()) {
    ...
    if($whyNot === true) {
        SelfHref::redirect($Qreq, [
            "p" => $prow->paperId,
            "m" => "edit",
            "corrupt_document" => isset($_GET['corrupt_document']),
            "update_dns" => isset($_GET['update_dns']),
        ]);
    }
...
if ($Qreq->updatecontacts && $Qreq->post_ok() && $prow) {
    if ($Me->can_administer($prow) || $Me->act_author_view($prow)) {
        ...
        if ($ps->prepare_save_paper_json($pj)) {
            if (!$ps->diffs)
                Conf::msg_warning($Conf->_("(2) No changes to submission #%d.", $prow->paperId));
