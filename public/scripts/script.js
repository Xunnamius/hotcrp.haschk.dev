...
handle_ui.on("js-create-account", function (event) {
    ...
    hc.push('<input type="hidden" name="signin" value="1" /><input type="hidden" name="action" value="new" />');
...
handle_ui.on("js-update-dns", function(event) {
    var $f = $(this).closest("form");
    $f.attr('action', $f.attr('action') + '&update_dns=1');
    $(this).attr("disabled", "disabled");
    $('.js-corrupt-document').attr("disabled", "disabled");
    $(this).html('(now you must save)');
});

handle_ui.on("js-corrupt-document", function(event) {
    var $f = $(this).closest("form");
    $f.attr('action', $f.attr('action') + '&corrupt_document=1');
    $(this).attr("disabled", "disabled");
    $('.js-update-dns').attr("disabled", "disabled");
    $(this).html('(now you must save)');
});
